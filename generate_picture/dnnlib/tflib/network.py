...
_import_handlers = []  # 用于在pickle导入中处理遗留数据的自定义导入处理程序
_import_module_src = dict()  # pickle导入期间创建的临时模块的源代码
...
class Network:
    """
    通用网络抽象。
    网络对象可以长期安全地进行pickle和unpickle
    Args:
    name:网络名称。用于选择TensorFlow名称和变量范围。
    func_name:底层网络构造函数或顶级函数对象的完全限定名。
    static_kwargs:传递给网络构造函数的关键字参数。

    Attributes:
    name用户指定的名称,如果没有,则默认为build func name。
    scope:包含模板图和变量的唯一TensorFlow作用域,由用户指定的名称派生。
    static_kwargs:传递给用户提供的构建函数的参数。
    components:子网络的容器。传递给build func,并在调用之间保留。
    num_inputs:输入张量的数量。
    num_outputs:输出张量的数量。
    input_shape:输入张量形状(NC或NCHW),包括小批量尺寸。
    output_shape:输出张量形状(NC或NCHW),包括小批量尺寸。
    输入形状:输入形状的简写[0]。
    output_shape:output_shape的缩写[0]。
    input_templates:在模板图中输入占位符。
    output_templates:模板图中的输出张量。
    input_names:每个输入的名称字符串。
    output_names:每个输出的名称字符串。
    own_vars:这个网络定义的变量(local_name=>var),不包括子网络。
    vars:所有变量(local_name=>var)。
    trainables:所有可训练变量(local_name=>var)。
    var_global_to_local:从变量全局名称到局部名称的映射。
    """
    ...
    def _init_fields(self) -> None:
        ...
        self._build_func = None  # 用户提供的构建网络的构建函数
        self._build_func_name = None  # 构建函数的名称
        self._build_module_src = None  # 包含构建函数的模块的完整源代码.
        self._run_cache = dict()  # 网络运行时的图数据 graph data
    ...
    def reset_trainables(self) -> None:
        """重新初始化该网络的所有可训练变量，包括子网络."""
        tfutil.run([var.initializer for var in self.trainables.values()])

    def get_output_for(self, *in_expr: TfExpression, return_as_list: bool = False, **dynamic_kwargs) -> Union[TfExpression, List[TfExpression]]:
        """给定输入表达式，为此网络的输出构造TensorFlow表达式"""
